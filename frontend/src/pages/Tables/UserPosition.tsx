...
const calculatePnL = (order: Order, cmp?: number) => {
...
export default function UserPosition() {
  ...
  const totalPNL = useMemo(() => {
    return filtered.reduce((sum, order) => {
      ...
      if (live !== undefined && price && qty) {
        const pnl = order.transactiontype === "BUY"
          ? (live - price) * qty
          : (price - live) * qty;
        return sum + pnl;
      }
  ...
  const getCMP = (order: Order) => {
    if (order.symboltoken && ltpByToken[order.symboltoken] !== undefined) {
      return ltpByToken[order.symboltoken];
    }
    return order.cmp !== undefined ? Number(order.cmp) : null;
  };

  const getPnL = (order: Order) => {
    const cmp = getCMP(order);
    if (cmp === null) return null;
    return calculatePnL(order, cmp);
  };
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 mb-3 shadow-sm hover:shadow-md transition-all">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm mb-3">
          <div>
            ...
            <p className="font-medium">₹{Number(order?.fillprice || 0).toFixed(2)}</p>
          </div>
          <div>
            <span className="text-gray-400 text-xs">LTP</span>
            <p className="font-medium">{cmp !== null ? `₹${Number(cmp).toFixed(2)}` : "—"}</p>
          </div>
          <div>
            <span className="text-gray-400 text-xs">PnL</span>
            <p className="font-medium">{pnlPill(pnl)}</p>
          </div>
          <div>
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm ml-12">
            Monitor your current positions and real-time P&L
          </p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">
                  {filtered.filter(o => o.status === "COMPLETE" || o.status === "complete").length}
                </p>
              </div>
