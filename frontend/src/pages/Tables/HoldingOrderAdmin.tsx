...
export default function HoldingOrderAdmin() {
  ...
  const handleSquareOff = async () => {
    const isConfirm = window.confirm("Are you sure you want to Square Off all holdings?");
    if (isConfirm) {
      try {
        const res = await axios.get(
          `${apiUrl}/admin/holding/sequareoff`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
        ...
        if (res.data.status) {
          toast.success(`Orders squared off successfully`);
        } else if (
          res.data.status === false &&
          res.data.message === "Unauthorized"
        ) {
  ...
  // Calculate total PNL and other stats
  ...
  const MobileOrderCard = ({ order }: { order: Order }) => {
    const live = order.symboltoken ? ltpByToken[String(order.symboltoken)] : undefined;
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Portfolio Holdings</h1>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-400 uppercase">Total Holdings</p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-400 uppercase">Buy Orders</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600 mt-1">
                  {new Set(filtered.map(o => o.userNameId)).size}
                </p>
              </div>
